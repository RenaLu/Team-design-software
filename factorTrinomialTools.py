...
#FACTOR THE QUADRATIC EXPRESSION WITH THREE TERMS THAT CAN BE FACTORED WITH QUADRATIC FORMULA      
def factorTrinomialThreeFormula (factorsA, factorsB, factorsC, factorsD):
    if factorsA==1:
        aTerm="x"
    else:
        aTerm=str(factorsA)+"x"

    if factorsB>0:
        bTerm="-"+str(factorsB)               
    elif factorsB<0:
        lengthB = len(str(factorsB))
        bTerm="+"+str(factorsB)[1:(lengthB)]

    if factorsC==1:
        cTerm="x"
    else:
        cTerm=str(factorsC)+"x"

    if factorsD>0:
        dTerm="-"+str(factorsD)               
    elif factorsD<0:
        lengthD = len(str(factorsD))
        dTerm= "+" + str(factorsD)[1:(lengthD)]
        
    #PERFECT SQUARE
    if factorsA==factorsC and factorsB==factorsD:
...
def factorTrinomialThree(coeA, coeB, coeC):
    commonFactor = findCommonFactorThree(coeA, coeB, coeC)
    newCoeA = coeA/commonFactor
    newCoeB = coeB/commonFactor
    newCoeC = coeC/commonFactor
    ...
    #IF THE TRINOMIAL CAN BE FACTORED INTO TWO SETS OF BRACKETS 
        ...
        return commonFactor + factorTrinomialThreeFormula(factors[0], factors[1], factors[2], factors[3])



#IF THE QUADRATIC EXPRESSION WITH A MISSING B-TERM
def factorTrinomialTwoB (coeA, coeC):
    commonFactor = findCommonFactorTwo(coeA,coeC)
    newCoeA = coeA/commonFactor
    newCoeC = coeC/commonFactor
...
#IF THE QUADRATIC EXPRESSION WITH A MISSING C-TERM
def factorTrinomialTwoC (coeA, coeB):                   
    commonFactor = findCommonFactorTwo (coeA,coeB)
    newCoeA= coeA/commonFactor
    newCoeB= coeB/commonFactor
...
def factorTrinomial (trinomial):
    ...
    if NumTerms == 3:
        return factorTrinomialThree (coefficients[0],coefficients[1],coefficients[2])
    
    elif NumTerms == 2:
        
        if coefficients[1]==0:
            return factorTrinomialTwoB (coefficients[0], coefficients[2])
        
        elif coefficients[2]==0:
            return factorTrinomialTwoC (coefficients[0], coefficients[1])
